...
import { cn } from "../../lib/utils"; // Adjusted relative path
...
>(
  (
    { title, subtitle, imageUrl, actionText, href, onActionClick, className, imageClassName },
    ref
  ) => {
    ...
    return (
      ...
      >
        ...
        >
          ...
          <img
            src={imageUrl}
            alt={`${title || 'Image'}, ${subtitle || ''}`}
            className={cn("absolute inset-0 h-full w-full rounded-xl object-cover", imageClassName)}
          />
          
          {/* Darkening overlay for better text contrast over the image - ONLY IF TITLE EXISTS */}
          {title && (
